...
static bool agent_use_ethernet = false;


bool uxr_init_udp_transport_datagram(uxrUDPTransportDatagram* transport)
{
    if(!agent_use_ethernet)
    {
        bool rv = false;
        /* Wifi connect initialization */
        rv = esp_connect_udp(ESP_MULTICAST_DEFAULT_IP, ESP_MULTICAST_DEFAULT_PORT);
        return rv;
    }
    else
    {
...
void uxr_choose_ethernet(bool use_ethernet)
{
    agent_use_ethernet = use_ethernet;
}
