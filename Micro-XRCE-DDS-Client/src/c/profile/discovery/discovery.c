        ...
        size_t agent_list_size)
{
    ...
    {
        for(uint32_t a = 0; a < attempts && !informed; ++a)
        {
            for(size_t i = 0; i < agent_list_size && !informed; ++i)
            {
            ...
            int poll = period;
            while(0 < poll && !informed)
            {
            	informed = listen_info_message(&transport, poll, &callback);
        ...
        CallbackData* callback)
{
    ...
    if(received)
    {
        ...
        {
        	informed = read_info_message(&ub, callback);
        }
    }
