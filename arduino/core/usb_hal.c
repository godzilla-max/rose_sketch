...
*******************************************************************************/
/* 
 * Modified 25 September 2014 by Koji Uchida: Change to the USB interrupt 64M.
 */ 
/*******************************************************************************
System Includes
*******************************************************************************/
...
static void WriteIntINPacket(void)
{
    ...
    {
        /* Create 8bit pointer to D1FIFO register */
        uint8_t * ptr = (uint8_t *)&USBIO.D0FIFO;	  /* kaede */
...
*                 Note: USB requires EXTAL = 12MHz.(RSK+24MHz)
*                 UCLK = EXTAL*4 = 48MHz.
* Argument      : none
* Return value  : none
**********************************************************************/
static void HW_Init(void)
{
    ...
    HW_Init_Module0();
     ...
     /*Start USB clock*/
    USBIO.SYSCFG.WORD |= 0x0400u;
    while (0x0400u != (USBIO.SYSCFG.WORD & 0x0400u))
    {
        /* Wait for Set of SCKE */
    }

    /*Enable USB operation*/
    USBIO.SYSCFG.WORD |= 0x0001u;
