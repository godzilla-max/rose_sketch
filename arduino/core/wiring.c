...
static inline unsigned long timerTicks()
{
	...
	unsigned long cmcnt = CMT1.CMCNT;
	int ir = IR(CMT1, CMI1);
	interrupts();
...
static void delayTicks(unsigned long ticks)
{
	if (!isNoInterrupts()) {
		...
		unsigned short s = CMT1.CMCNT;
		for (;;) {
			unsigned short l = CMT1.CMCNT;
...
unsigned long micros() {
	...
	unsigned long cmcnt = CMT1.CMCNT;
	int ir = IR(CMT1, CMI1);
	if (ir && cmcnt < 1) { // avoid boundary problem
		ms++;
	}
	if (!di) {
		interrupts();
...
void init()
{
	...
	CMT1.CMCR.WORD = 0;
	CMT1.CMCR.BIT.CKS = 0b00;	// PCLK/8
	CMT1.CMCR.BIT.CMIE = 1;	// compare match interrupt enable
