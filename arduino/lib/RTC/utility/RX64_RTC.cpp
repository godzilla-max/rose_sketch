...
/**
 * @file  RTC.cpp
 * @brief RX64Mマイコン内蔵の時計機能（RTC：リアル・タイム・クロック）を使うためのライブラリです。
 *
...
int rtc_set_time(RTC_TIMETYPE* time)
{
    ...
    /* Operate Calendar mode*/
    RTC.RCR2.BIT.CNTMD = 0x0;

    /* Operate RTC in 24-hr mode */
    RTC.RCR2.BIT.HR24 = 0x1;

    RTC.RYRCNT.WORD  = HEX2BCD(time->year % 100);
...
{
    ICU.IER[IER_PERIB_INTB135].BIT.IEN4 = 0; // RTC, CUP
    RTC.RCR1.BIT.CIE = 1;
    do {
        ICU.IR[IR_PERIB_INTB135].BIT.IR = 0;
        time->year    = BCD2HEX(RTC.RYRCNT.WORD) + 2000;
    ...
    } while (ICU.IR[IR_PERIB_INTB135].BIT.IR);
    RTC.RCR1.BIT.CIE = 0;
    return 1;
}
...
{
    return BCD2HEX(RTC.RYRCNT.WORD) + 2000;
}

unsigned char rtc_get_mon()
{
    return BCD2HEX(RTC.RMONCNT.BYTE);
}

unsigned char rtc_get_day()
{
    return BCD2HEX(RTC.RDAYCNT.BYTE);
}

unsigned char rtc_get_hour()
{
    return BCD2HEX(0x3f & RTC.RHRCNT.BYTE);
}

unsigned char rtc_get_min()
{
    return BCD2HEX(RTC.RMINCNT.BYTE);
}

unsigned char rtc_get_second()
{
    return BCD2HEX(RTC.RSECCNT.BYTE);
}

unsigned char rtc_get_weekday()
{
    return BCD2HEX(RTC.RWKCNT.BYTE);
}
 ...
 * @attention なし
 ***************************************************************************/
void isr_rtc_alarm(void) __attribute__((interrupt(".rvectors", VECT(RTC, ALM)), used));
