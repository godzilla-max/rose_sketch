...
#else
static uint8_t i2c_rxBuffer[IIC_CHANNEL_NUM][BUFFER_LENGTH];
...
void TwoWire::begin(void)
{
  ...
  if(wire_channel == 0){ // if Software I2C
      ...
      twi_rx_init(wire_channel, wire_frequency);
  }
}
#endif
...
uint8_t TwoWire::requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop)
{
  ...
  if (wire_channel == 0) {
      ...
      if (g_sci_i2c_channel_inRepStart[wire_channel] != true) {
          twi_rx_start(wire_channel, address);
      } else {
          twi_rx_restart(wire_channel, address);
      }
      for (int i = 0; i < quantity; i++) {
          ...
          i2c_rxBuffer[wire_channel][i] = twi_rx_read(
                  wire_channel, i == (quantity - 1));
          read++;
      }
      if (sendStop == true) {
          twi_rx_stop(wire_channel);
          g_sci_i2c_channel_inRepStart[wire_channel] = false;
      } else {
          g_sci_i2c_channel_inRepStart[wire_channel] = true;
      }
...
size_t TwoWire::write(uint8_t data)
{
  ...
  // in slave send mode
    // reply to master
      twi_rx_write(wire_channel, data);
  }
  return 1;
}
...
void TwoWire::onReceiveService(uint8_t* inBytes, int numBytes)
{
  ...
  for(uint8_t i = 0; i < numBytes; ++i){
      i2c_rxBuffer[wire_channel][i] = inBytes[i];
  }
